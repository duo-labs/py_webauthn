...
# Only supporting 'fido-u2f' attestation format for now.
...
class WebAuthnRegistrationResponse(object):
    ...
    def verify(self):
        ...
        verifier = pk.verifier(signature, ECDSA(SHA256()))
        verifier.update(bytes_to_sign)
        try:
            verifier.verify()
        ...
        if attestation_type == AT_SELF_ATTESTATION:
            if _self_attestation_permitted():
        ...
        # Step 15.
        #
        # If verification of the attestation statement failed, the Relying
...
class WebAuthnAssertionResponse(object):
    ...
    def verify(self):
        ...
        # of aData and hash.
        decoded_user_pub_key = _decode_public_key(base64.b64decode(self.webauthn_user.public_key))
        user_pubkey = decoded_user_pub_key.public_key(backend=default_backend())
        bytes_to_sign = ''.join([
            decoded_a_data,
            client_data_hash])
        verifier = user_pubkey.verifier(sig, ECDSA(SHA256()))
        verifier.update(bytes_to_sign)
        try:
            verifier.verify()
        except InvalidSignature:
...
def _self_attestation_permitted():
